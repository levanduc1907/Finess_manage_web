...
import { useCreateSchedule } from '../modules/useCreateSchedule';
import { useUpdateSchedule } from '../modules/useUpdateSchedule';

export function ScheduleNewSubmit() {
  ...
  const { create, status: createStatus } = useCreateSchedule();
  const { update, status: updateStatus } = useUpdateSchedule();
  const isEditScreen = !!params?.id;
  const onSubmit = handleSubmit(async (value: any) => {
    try {
      let promiseRes: Promise<TFetchBaseOutput<TSchedule>>;
      if (isEditScreen) {
        const body = {
          id: params.id,
          name: value?.name,
      ...
      queryClient.refetchQueries({
        queryKey: [QueryKeys.GET_SCHEDULE],
      });
      navigate('/schedule');
    } catch (e) {
